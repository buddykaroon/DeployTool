SUBFILENAME=StandardRetrieve-$(date +%Y%m%d%H%M%S)
sfdx auth:list
DEVUSERNAME=sopon.s@tcb.dev

echo "Creating Project"
cd ./retrieves
sfdx force:project:create -n $SUBFILENAME
cd $SUBFILENAME
mkdir manifest
cp ../../packages/sourcePackage.xml ./manifest/sourcePackage.xml
cp ../../packages/sourcePackage.xml ./manifest/destinationPackage.xml
node ../../helper/fixProfile.js ./manifest/sourcePackage.xml

echo "$1"

sfdx force:source:retrieve -x ./manifest/sourcePackage.xml -u $DEVUSERNAME > sfdxDevRetrieveLogs.txt

# sopon.s - to fix RouteWork QueueId
node ../../helper/fixRouteWork.js ./retrieves/$SUBFILENAME "TCB_Email_to_Case_CR_18" "TCB_Case_Assignment"
# ==================================


cd ../..



if [ "$1" == "-validate" ]; then 
    ./validateSit.sh ./retrieves/$SUBFILENAME
fi
# xcopy $SUBFILENAME  $SUBFILENAME-raw  /h /i /c /k /e /r /y
